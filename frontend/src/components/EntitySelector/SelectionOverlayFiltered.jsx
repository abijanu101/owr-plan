...
// Data fetching logic (kept internal for now)
...
export default function SelectionOverlayFiltered({ isOpen, onClose, selectedIds, onToggle, People, entities = [], loading = false, error = null }) {
    ...
    const handleSelectAll = (sectionId, sectionEntities) => {
        const entityIds = sectionEntities.map(e => String(e.id));
                        ...
                        sections.map((section, index) => {
                            const sectionEntities = filteredEntities.filter(e => {
                                if (section.type === 'group_members') {
                                    return section.memberIds.includes(String(e.id));
                                }
                            ...
                            return (
                                <div
                                    key={section.id}
                                    className="group/item transition-all duration-300 rounded-[20px] border border-[#DC8379]/10"
                                >
                                    {/* Section Header */}
                                    <div className="flex items-center gap-4 px-4 py-2 relative cursor-pointer hover:bg-white/5 transition-colors rounded-t-[20px]" onClick={() => toggleSection(section.id)}>
                                        <h3 className="text-[18px] sm:text-[24px] font-normal text-[#DC8379] tracking-wide flex-1 pl-4" style={{ fontFamily: 'cursive' }}>
                                            {section.title}
                                        </h3>
                                                ...
                                                <div className="flex flex-wrap gap-2 md:gap-3 flex-1">
                                                    {sectionEntities.map(entity => (
                                                        <EntityChip
                                                            ...
                                                            name={entity.name}
                                                            color={entity.color}
                                                            isSelected={selectedIds.includes(String(entity.id))}
